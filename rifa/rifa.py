from rifa_utils import escrever_tentativas_vendas, bye, filtrar_pontos_vendidos, filtrar_pontos_nao_vendidos, calcular_valor_arrecadado, mostrar_todos_vencedores
...
def main():
    ...
    while opcao != 0:
        if opcao == 1:
            pontos_disponiveis = obter_numero_positivo('Pontos disponívies: ')
            ...
            valor_ponto = obter_numero_positivo("> ")
            mostrar_texto('Valor atualizado!')
        elif opcao == 4:
            taxa_administracao = obter_numero_float_positivo("> ")
            mostrar_texto('Valor atualizado!')
        elif opcao == 5:
            valor_arrecadado = calcular_valor_arrecadado(valor_ponto, pontos_vendidos)
            total_administracao = valor_arrecadado*taxa_administracao
            mostrar_texto(f'O total arrecadado foi: {valor_arrecadado}')
            mostrar_texto(f'O total da taxa de administração foi: {total_administracao}')
        elif opcao == 6:
...
def menu():
    ...
    opcoes += "\n1 - Informar a quantidade de pontos disponivéis para venda"
